...
self.restaurants = []; // Keep a list of recently fetched restaurants during first page load.
const storeRestaurants = (restaurants) => {
  self.restaurants = restaurants;
  return restaurants; // pipe argument back
};
self.waitForFetch = false; // To check if a fetch is bing triggered, and to avoid another one.
/**
 * 
 * @param {Function} callback Callback function... yes a callback. It will execute once self.restaurants is populated ( > 0)
 * @param {*} delay Time in ms to delay checking if fetch is done. Default is 100ms.
 */
function waitFetch(callback, delay = 100) {
  const wait = function() {
...
document.addEventListener('DOMContentLoaded', (event) => {
  // If window.initMap executed first, then restaurants may be in the process of being fetched,
  // or are done and stored in self.restaurants. I
  if (self.waitForFetch) {
    // wait until self.restaurants is populated...
    waitFetch(function() {
      Promise.resolve(self.restaurants)
  ...
  // If this function executes before initMap, then store restaurant list in self.restaurants right after
  // fetching the restaurants.
  self.waitForFetch = true; // let initMap know it should wait for fetch
...
window.initMap = () => {
  ...
  // if restaurants are in the proccess of being fetched, or were fetched already,
  // use them instead of fetching them again.
  if (self.waitForFetch) {
    // wait until self.restaurants is populated...
    waitFetch(function() {
      console.log("initMap waiting...");
      return Promise.resolve(self.restaurants)
      .then(addMarkersToMap)
      .catch(console.error);
    });
    return;
  }
  // If initMap executes before DOMContentLaoded is triggered, fetch restaurants and store them
  // in self.restaurants.
  self.waitForFetch = true; // let DOMContentLoaded know to wait for a fetch
  DBHelper.fetchRestaurantsPromise()
    .then(storeRestaurants)
    .then(addMarkersToMap)
    .catch(console.error);
